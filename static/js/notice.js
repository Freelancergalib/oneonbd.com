...
function updateTable(){
	$(document).ready(()=>{
		$.ajax({
			type:'POST',
			url:'function_manage_notice.php',
			data: {cmd:'show_table'},
			success:(response)=>{
...
$(document).ready(()=>{
	...
	$('form').on('submit',e=>{
		...
		let title = $('input[name=notice]').val()
		...
		//updating data of the database
			...
			$.ajax({
				type:'POST',
				url:'function_manage_notice.php',
				data: {cmd:'update',pk:parseInt(dbId),newTitle:title},
				success:(response)=>{
					updateTable()
				},
				error:(err)=>{
